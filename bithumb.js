...
function XCoinAPI (account) {
  this.apiUrl = 'https://api.bithumb.com'
	this.api_key = account.apiKey
	this.api_secret = account.apiSecret
}
...
XCoinAPI.prototype.getAccount = function () {
  var rgParams = {
    order_currency: 'BTC',
    payment_currency: 'KRW'
  }
  return this.xcoinApiCall('/info/account', rgParams)
    .then(account => {
      return [
        {
          currency: 'BTC',
          balance: parseFloat(account.data.balance)
        }
      ];
    })
}

XCoinAPI.prototype.getBalance = function () {
  var rgParams = {
    order_currency: 'BTC',
    payment_currency: 'KRW'
  }
...
var usec = rgMicrotime[0];

		
var sec = rgMicrotime[1]
...
function base64_encode (data) {
  ...
  var b64 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/='
	var o1; var o2; var o3; var h1; var h2; var h3; var h4; var bits; var i = 0;
...
*/
var CryptoJS = CryptoJS || function (a, j) { var c = {}; var b=c.lib={}; var f=function(){}; var l=b.Base={extend:function(a){f.prototype=this;var d=new f;a&&d.mixIn(a);d.hasOwnProperty("init")||(d.init=function(){d.$super.init.apply(this,arguments)});d.init.prototype=d;d.$super=this;return d},create:function(){var a=this.extend();a.init.apply(a,arguments);return a},init:function(){},mixIn:function(a){for(var d in a)a.hasOwnProperty(d)&&(this[d]=a[d]);a.hasOwnProperty("toString")&&(this.toString=a.toString)},clone:function(){return this.init.prototype.extend(this)}};


var u=b.WordArray=l.extend({init:function(a,d){a=this.words=a||[];this.sigBytes=d!=j?d:4*a.length},toString:function(a){return(a||m).stringify(this)},concat:function(a){var d=this.words,M=a.words,e=this.sigBytes;a=a.sigBytes;this.clamp();if(e%4)for(var b=0;b<a;b++)d[e+b>>>2]|=(M[b>>>2]>>>24-8*(b%4)&255)<<24-8*((e+b)%4);else if(65535<M.length)for(b=0;b<a;b+=4)d[e+b>>>2]=M[b>>>2];else d.push.apply(d,M);this.sigBytes+=a;return this},clamp:function(){var D=this.words,d=this.sigBytes;D[d>>>2]&=4294967295<<
32-8*(d%4);D.length=a.ceil(d/4)},clone:function(){var a=l.clone.call(this);a.words=this.words.slice(0);return a},random:function(D){for(var d=[],b=0;b<D;b+=4)d.push(4294967296*a.random()|0);return new u.init(d,D)}}); var k=c.enc={}; var m=k.Hex={stringify:function(a){var d=a.words;a=a.sigBytes;for(var b=[],e=0;e<a;e++){var c=d[e>>>2]>>>24-8*(e%4)&255;b.push((c>>>4).toString(16));b.push((c&15).toString(16))}return b.join("")},parse:function(a){for(var d=a.length,b=[],e=0;e<d;e+=2)b[e>>>3]|=parseInt(a.substr(e,
2),16)<<24-4*(e%8);return new u.init(b,d/2)}}; var y=k.Latin1={stringify:function(a){var b=a.words;a=a.sigBytes;for(var c=[],e=0;e<a;e++)c.push(String.fromCharCode(b[e>>>2]>>>24-8*(e%4)&255));return c.join("")},parse:function(a){for(var b=a.length,c=[],e=0;e<b;e++)c[e>>>2]|=(a.charCodeAt(e)&255)<<24-8*(e%4);return new u.init(c,b)}}; var z=k.Utf8={stringify:function(a){try{return decodeURIComponent(escape(y.stringify(a)))}catch(b){throw Error("Malformed UTF-8 data");}},parse:function(a){return y.parse(unescape(encodeURIComponent(a)))}};


var x = b.BufferedBlockAlgorithm = l.extend({reset: function () { this._data = new u.init(); this._nDataBytes = 0 }, _append: function (a) { "string" == typeof a && (a = z.parse(a)); this._data.concat(a); this._nDataBytes += a.sigBytes },
_process: function (b) { var d = this._data; var c=d.words; var e=d.sigBytes; var l=this.blockSize; var k=e/(4*l); var k = b ? a.ceil(k):a.max((k | 0) - this._minBufferSize, 0); b = k * l; e = a.min(4 * b, e); if (b) { for (var x = 0; x < b; x += l) this._doProcessBlock(c, x); x = c.splice(0, b); d.sigBytes -= e } return new u.init(x, e) },
clone: function () {
var a = l.clone.call(this)
a._data = this._data.clone(); return a }, _minBufferSize: 0}); b.Hasher = x.extend({cfg: l.extend(), init: function (a) { this.cfg = this.cfg.extend(a); this.reset() }, reset: function () { x.reset.call(this); this._doReset() },
update: function (a) { this._append(a); this._process(); return this },
finalize: function (a) { a && this._append(a); return this._doFinalize() }, blockSize: 16,
_createHelper: function (a) { return function (b, c) { return (new a.init(c)).finalize(b) } },
_createHmacHelper: function (a) { return function (b, c) { return (new ja.HMAC.init(a,
  c)).finalize(b) }
}}); var ja = c.algo = {}; return c }(Math);
(function (a) { var j = CryptoJS; var c=j.lib; var b=c.Base; var f=c.WordArray; var j = j.x64 = {}; j.Word = b.extend({init: function (a, b) { this.high = a; this.low = b }}); j.WordArray = b.extend({init: function (b, c) { b = this.words = b || []; this.sigBytes = c != a ? c:8 * b.length }, toX32: function () { for (var a = this.words, b = a.length, c = [], m = 0; m < b; m++) { var y = a[m]; c.push(y.high); c.push(y.low) } return f.create(c, this.sigBytes) }, clone: function () { for (var a = b.clone.call(this), c = a.words = this.words.slice(0), k = c.length, f = 0; f < k; f++)c[f] = c[f].clone(); return a }}) })();
...
var A = k[t]; if (t<16) var q = A.high = a[b + 2 * t] | 0; var g = A.low = a[b + 2 * t + 1] | 0; else { var q = k[t - 15]; var g=q.high; var v=q.low; var q=(g>>>1|v<<31)^(g>>>8|v<<24)^g>>>7; var v=(v>>>1|g<<31)^(v>>>8|g<<24)^(v>>>7|g<<25); var C=k[t-2]; var g=C.high; var h=C.low; var C=(g>>>19|
h<<13)^(g<<3|h>>>29)^g>>>6; var h=(h>>>19|g<<13)^(h<<3|g>>>29)^(h>>>6|g<<26); var g=k[t-7]; var $=g.high; var B=k[t-16]; var w=B.high; var B=B.low; var g=v+g.low; var q=q+$+(g>>>0<v>>>0?1:0); var g=g+h; var q=q+C+(g>>>0<h>>>0?1:0); var g=g+B; var q = q + w + (g >>> 0 < B >>> 0 ? 1:0); A.high = q; A.low = g } var $ = s & W ^ ~s & X; var B=p&J^~p&K; var A=r&G^r&H^G&H; var ka=n&E^n&F^E&F; var v=(r>>>28|n<<4)^(r<<30|n>>>2)^(r<<25|n>>>7); var C=(n>>>28|r<<4)^(n<<30|r>>>2)^(n<<25|r>>>7); var h=u[t]; var la=h.high; var ia=h.low; var h=L+((p>>>14|s<<18)^(p>>>18|s<<14)^(p<<23|s>>>9)); var w=Z+((s>>>14|p<<18)^(s>>>18|p<<14)^(s<<23|p>>>9))+(h>>>
0<L>>>0?1:0); var h=h+B; var w=w+$+(h>>>0<B>>>0?1:0); var h=h+ia; var w=w+la+(h>>>0<ia>>>0?1:0); var h=h+g; var w=w+q+(h>>>0<g>>>0?1:0); var g=C+ka; var A=v+A+(g>>>0<C>>>0?1:0); var Z=X; var L=K; var X=W; var K=J; var W=s; var J=p; var p=I+h|0; var s=Y+w+(p>>>0<I>>>0?1:0)|0; var Y=H; var I=F; var H=G; var F=E; var G=r; var E=n; var n=h+g|0; var r = w + A + (n >>> 0 < h >>> 0 ? 1:0) | 0
...
}, _doFinalize: function () { var a = this._data; var b=a.words; var c=8*this._nDataBytes; var f = 8 * a.sigBytes; b[f >>> 5] |= 128 << 24 - f % 32; b[(f + 128 >>> 10 << 5) + 30] = Math.floor(c / 4294967296); b[(f + 128 >>> 10 << 5) + 31] = c; a.sigBytes = 4 * b.length; this._process(); return this._hash.toX32() }, clone: function () { var a = c.clone.call(this); a._hash = this._hash.clone(); return a }, blockSize: 32}); j.SHA512 = c._createHelper(b); j.HmacSHA512 = c._createHmacHelper(b) })();
(function () {
var a = CryptoJS; var j = a.enc.Utf8; a.algo.HMAC = a.lib.Base.extend({init: function (a, b) { a = this._hasher = new a.init(); "string" == typeof b && (b = j.parse(b)); var f = a.blockSize; var l = 4 * f; b.sigBytes > l && (b = a.finalize(b)); b.clamp(); for (var u = this._oKey = b.clone(), k = this._iKey = b.clone(), m = u.words, y = k.words, z = 0; z < f; z++)m[z] ^= 1549556828, y[z] ^= 909522486; u.sigBytes = k.sigBytes = l; this.reset() }, reset: function () { var a = this._hasher; a.reset(); a.update(this._iKey) },
