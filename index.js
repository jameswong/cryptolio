...
app.get('/', (req, res) => {
    ...
    .then(tickers => {
        var [btc, bch, eth, ltc, bittrexAll, binanceAll, total] = tickers;
        var prices = {};
        ...
        for (const currency in total) {
            switch (currency) {
                case 'USD':
                    sum.USD += total['USD'].balance;
                    break;
                case 'BTC':
                    sum.BTC += total['BTC'].balance;
                case 'BCH':
                case 'ETH':
                case 'LTC':
                    total[currency].USD = prices[currency + 'USD']
                    total[currency]['usd-value'] = total[currency].balance * prices[currency + 'USD'].price
                    sum.USD += total[currency]['usd-value'];
                    break;
                default:
                    total[currency].BTC = prices[currency + 'BTC']
                    total[currency]['btc-value'] = total[currency].balance * prices[currency + 'BTC'].price
                    sum.BTC += total[currency]['btc-value'];
            }
        }

        sum.USD += sum.BTC * prices['BTCUSD'].price;
        console.log(JSON.stringify(total, null, 2));
        console.log(JSON.stringify(sum, null, 2));
        res.send('Crypto Portfolio');
    })
    .catch(err => {
        res.send(err);
    });
